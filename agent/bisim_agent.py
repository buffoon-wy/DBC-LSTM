...
class BisimAgent(object):
    ...
    def __init__(
        ...
        self.actor = Actor(
            device, obs_shape, action_shape, hidden_dim, encoder_type,
    ...
    def select_action(self, obs, o_buff, a_buff, o_buff_len):
        with torch.no_grad():
            obs = torch.FloatTensor(obs).to(self.device)
            obs = obs.unsqueeze(0)
            hist_obs = torch.tensor(o_buff).float().to(self.device)
            hist_act = torch.tensor(a_buff).float().to(self.device)
            hist_seg_len = torch.tensor([o_buff_len]).float().to(self.device)

            mu, _, _, _ = self.actor(obs, hist_obs, hist_act, hist_seg_len, compute_log_pi=False)
            return mu.cpu().data.numpy().flatten()

    def sample_action(self, obs, o_buff, a_buff, o_buff_len):
        with torch.no_grad():
            obs = torch.FloatTensor(obs).to(self.device)
            obs = obs.unsqueeze(0)
            hist_obs = torch.tensor(o_buff).float().to(self.device)
            ...
            mu, pi, _, _ = self.actor(obs, hist_obs, hist_act, hist_seg_len, compute_log_pi=False)
            # print("pi = ", pi)
            # pi_modified = pi.cpu().data.numpy().flatten()
            # pi_modified[1] = 1
            return pi.cpu().data.numpy().flatten()

    def update_critic(self, obs, act, reward, next_obs,  h_o, h_a, h_o2, h_a2,  h_o_len, h_o2_len, not_done, L, step):
        with torch.no_grad():
            _, policy_action, log_pi, _ = self.actor(obs, h_o, h_a, h_o_len)
            target_Q1, target_Q2 = self.critic_target(next_obs, policy_action, h_o2, h_a2, h_o2_len)
            target_V = torch.min(target_Q1,
                                 target_Q2) - self.alpha.detach() * log_pi
            target_Q = reward + (not_done * self.discount * target_V)

        # get current Q estimates
        current_Q1, current_Q2 = self.critic(obs, act, h_o, h_a, h_o_len, detach_encoder=False)
    ...
    def update_actor_and_alpha(self, obs, act, h_o, h_a, h_o_len, L, step):
        # detach encoder, so we don't update it with the actor loss
        _, pi, log_pi, log_std = self.actor(obs, h_o, h_a, h_o_len, detach_encoder=True)
        actor_Q1, actor_Q2 = self.critic(obs, pi, h_o, h_a, h_o_len, detach_encoder=True)
    ...
    def update_encoder(self, obs, h_o, h_a, h_o_len, action, reward, L, step):
        h = self.critic.encoder(obs, h_o, h_a, h_o_len)
    ...
    def update_transition_reward_model(self, obs, next_obs, h_o, h_a, h_o2, h_a2, h_o_len, h_o2_len, action, reward, L, step):
        h = self.critic.encoder(obs, h_o, h_a, h_o_len)
        pred_next_latent_mu, pred_next_latent_sigma = self.transition_model(torch.cat([h, action], dim=1))
        if pred_next_latent_sigma is None:
            pred_next_latent_sigma = torch.ones_like(pred_next_latent_mu)

        next_h = self.critic.encoder(next_obs, h_o2, h_a2, h_o2_len)
    ...
    def update(self, replay_buffer, L, step, max_hist_len):
        # get data from the replay_buffer
        batch = replay_buffer.sample_batch_with_history(max_hist_len=max_hist_len)
        batch = {k: v.to(self.device) for k, v in batch.items()}
        obs, act, reward, next_obs, done = batch['obs'], batch['act'], batch['rew'], batch['obs2'], batch['done']
        h_o, h_a, h_o2, h_a2, h_o_len, h_o2_len = batch['hist_obs'], batch['hist_act'], batch['hist_obs2'], batch[
            'hist_act2'], batch['hist_obs_len'], batch['hist_obs2_len']

        L.log('train/batch_reward', reward.mean(), step)

        self.update_critic(obs, act, reward, next_obs, h_o, h_a, h_o2, h_a2, h_o_len, h_o2_len, done, L, step)
        transition_reward_loss = self.update_transition_reward_model(
            obs, next_obs, h_o, h_a, h_o2, h_a2, h_o_len, h_o2_len, act, reward, L, step)
        encoder_loss = self.update_encoder(obs, h_o, h_a, h_o_len, act, reward, L, step)
        ...
        if step % self.actor_update_freq == 0:
            self.update_actor_and_alpha(obs, act, h_o, h_a, h_o_len, L, step)
